import {
  GW_STATUS, GwStatus,
  ConnectErrorCodes, ConnectErrorCode,
  CLOSE_CODE_HINTS,
  GatewayHelloOk, GatewayErrorInfo,
  EventHandler,
} from './types.js';
...
// ============================================================
// Device Identity（移植自官方 device-identity.ts，纯浏览器 SubtleCrypto 实现）
// ============================================================

interface StoredDeviceIdentity {
  version: 1;
  deviceId: string;
  publicKey: string;  // base64url Ed25519 公钥（32 字节）
  privateKey: string; // base64url Ed25519 私钥（32 字节）
  createdAtMs: number;
}
...
async function fingerprintPublicKey(pubKeyBytes: Uint8Array): Promise<string> {
  const hash = await crypto.subtle.digest('SHA-256', pubKeyBytes.buffer as ArrayBuffer);
...
async function loadOrCreateDeviceIdentity(): Promise<StoredDeviceIdentity | null> {
  if (typeof crypto === 'undefined' || !crypto.subtle) return null;
  try {
    const raw = localStorage.getItem(DEVICE_IDENTITY_KEY);
    if (raw) {
      const parsed: StoredDeviceIdentity = JSON.parse(raw);
...
async function signDevicePayload(privateKeyB64url: string, payload: string): Promise<string | null> {
  try {
    ...
    const sig = await crypto.subtle.sign({ name: 'Ed25519' } as AlgorithmIdentifier, key, data);
...
function buildDeviceAuthPayload(params: {
  ...
  role: string; scopes: string[]; signedAtMs: number; token: string | null; nonce: string;
}): string {
  return [
    'v2',
    params.deviceId,
...
const AUTH_ERROR_CODES = new Set<string>(Object.values(ConnectErrorCodes));
...
export interface GatewayClientOptions {
  ...
  token?: string;
  password?: string;
  reconnectDelay?: number; // 向后兼容，被指数退避覆盖
}
...
export class GatewayClient extends EventEmitter {
  ...
  private _status: GwStatus = GW_STATUS.DISCONNECTED;
  private _handshakeDone = false;
  private _reconnectTimer: ReturnType<typeof setTimeout> | null = null;
  private _stopped = true; // 是否主动停止（阻止自动重连）

  /** 指数退避当前值 */
  private _backoff = BACKOFF_INITIAL;
  ...
  private _setStatus(status: GwStatus): void {
  ...
  private _startConnectionFlow(): void {
    this._stopped = false;
    this._clearReconnectTimer();
    this._teardownSocket(true);
    this._handshakeDone = false;
  ...
  private _teardownSocket(suppressCloseHandler = false): void {
    if (!this._ws) return;
  ...
  call(method: string, params: Record<string, any> = {}, timeout = 10000): Promise<any> {
  ...
  async sendInstruction(sessionKey: string, message: string): Promise<any> {
  ...
  async abortSession(sessionKey: string): Promise<any> {
    return this.call('chat.abort', { sessionKey });
  }

  async resolveApproval(approvalId: string, decision = 'allow-once'): Promise<any> {
  ...
  async sendAction(action: string, sessionId: string, instruction: string): Promise<any> {
    return this.call('clawdeck.action', { action, sessionId, instruction });
  }

  async pollLogs(sinceId = 0): Promise<any> {
    return this.call('clawdeck.logs', { sinceId });
  }

  /** 向 idle Agent 发送消息并创建新会话（对应 Gateway `agent` RPC） */
  async startAgentChat(agentId: string, message: string): Promise<any> {
  ...
  private _handleMessage(msg: any): void {
    ...
    if (msg.type === 'res' && msg.id != null) {
      ...
      if (msg.ok) {
        ...
        const err = new Error(msg.error?.message ?? 'RPC error') as any;
  ...
  private async _sendConnect(nonce: string): Promise<void> {
    const id = String(++this._requestId);
    ...
    this._pending.set(id, {
      resolve: (payload: any) => {
        // 保存 hello-ok 数据
        this.hello = payload as GatewayHelloOk;
  ...
  private _flushPendingErrors(reason: string): void {
