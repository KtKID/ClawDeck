...
默认产物目录是 `./dist/plugin-package/clawdeck`。打包步骤默认只会写入这个项目内目录，不会直接安装到你的 OpenClaw 主目录。

普通用户不要再使用 `install.sh`，也不要直接执行 `node scripts/deploy-plugin.mjs`。这两条路径都不再作为正式安装入口，其中 `install.sh` 已废弃，仅保留给内部开发或历史兼容排查参考。

打开 `http://localhost:<端口>/plugins/clawdeck/` 即可使用（运行 `openclaw web` 查看实际端口）。

### 开发态 link 安装（仅开发调试）

如果你需要开发态调试，可使用官方 link 模式：
...
`-l` 表示以 link 方式安装，适合开发调试，不是普通用户推荐的正式安装入口。仍建议先生成标准插件目录，并让 link 指向 `./dist/plugin-package/clawdeck`，不要直接指向仓库源码根目录。
